<template>
  <div>
    ...
    <div class="card" style="margin-bottom: 5px">
      <el-input v-model="data.name" prefix-icon="Search" style="width: 240px; margin-right: 10px" placeholder="请输入公告标题查询"></el-input>
...
const delBatch = () => {
  ...
  ElMessageBox.confirm(
    ...
    try {
      // 2. 重点：POST请求，路径改为/notice/batch/delete
      const res = await request.post("/notice/batch/delete", data.ids);
      if (res.code === '200') {
...
const add = () => {
  // 1. 打印请求信息，确认参数和路径
  console.log("请求路径：/notice/add，请求参数：", data.form);

  request.post("/notice/add", data.form)
      .then(res => {
        if(res.code === '200'){
...
const update = async (formData) => {
  ...
  try {
    const res = await request.put('/notice/update', formData);
    if (res.code === '200') {
