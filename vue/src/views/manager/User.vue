...
<!--    输入表数据-->
    <el-dialog title="普通用户的信息" v-model="data.formVisible" width="40%" destroy-on-close>
      ...
      <el-form ref="form" :model="data.form.avatar" label-width="70px" style="padding:20px">
        ...
        <el-form-item prop="account" label="余额">
          <el-input-number  v-model="data.form.account" placeholder="请输入余额"></el-input-number>
          </el-form-item>
      </el-form>
      <template #footer>
...
const delBatch = () => {
  ...
  ElMessageBox.confirm(
    ...
    try {
      // 2. 重点：POST请求，路径改为/user/batch/delete
      const res = await request.post("/user/batch/delete", data.ids);
      if (res.code === '200') {
...
const add=()=>{
  request.post("/user/add",data.form).then(res=>{///user/add在UserController函数中定义
    if(res.code==='200'){
...
const update = async (formData) => {
  ...
  try {
    const res = await request.put('/user/update', formData);
    if (res.code === '200') {
